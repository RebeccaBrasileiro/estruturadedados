...
class BinaryTree:
    ...
    # Calcula a altura da árvore ou de um nó específico
    def height(self, node=None):
        ...
        if node.left is None and node.right is None:
            return 1  # Nó folha
        hleft = self.height(node.left) if node.left else 0
        hright = self.height(node.right) if node.right else 0
...
if __name__ == '__main__':
    ...
    no_menos = tree.root.right.right  # Nó '-'
